...
const DELAY_MS = 600;
const LOG_FILE = 'build-log.json';

function appendLog(status, cardCount = 0, errorMsg = null) {
  let logs = [];
  try {
    ...
    console.error('Warning: Failed to read existing log file. Starting fresh log array.', err);
  }
...
async function fetchAllCards() {
  try {
    ...
    appendLog('success', finalCardsArray.length, null);
  } catch (error) {
    console.error('Fatal extraction error:', error);
    appendLog('failure', 0, error.message);
    process.exit(1);
  }
}
